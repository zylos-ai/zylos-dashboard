...
async function requirePrivateDirectory(directory) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink() || (stat.mode & 0o077) !== 0 ||
    ...
    throw new Error('directory is not owned and private');
  }
}
...
async function readPrivateJson(file) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || (stat.mode & 0o077) !== 0 ||
      (typeof process.getuid === 'function' && stat.uid !== process.getuid())) throw new Error('unsafe ownership file');
  return JSON.parse(await fs.promises.readFile(file, 'utf8'));
}

async function validateSocketNamespace(namespace, runtimeRoot) {
  if (typeof namespace?.root !== 'string' || !/^\/tmp\/zobs-[A-Za-z0-9]{6}$/.test(namespace.root) ||
      typeof namespace.token !== 'string' || !/^[0-9a-f]{32}$/.test(namespace.token)) {
    throw new Error('invalid socket namespace');
  }
  try { await requirePrivateDirectory(namespace.root); }
  catch (error) { if (error?.code === 'ENOENT') return false; throw error; }
  const owner = await readPrivateJson(path.join(namespace.root, '.observer-namespace.json'));
  if (owner.runtimeRoot !== runtimeRoot || owner.token !== namespace.token) throw new Error('socket namespace ownership mismatch');
  return true;
}

async function validatePersistedSocketRoot(state, retired = false) {
  const { socketRoot, socketOwnerFile, parent, nonce, marker, root, socketNamespace } = state;
  const expectedOwnerFile = path.join(socketRoot || '', '.observer-owner.json');
  const validPath = socketNamespace
    ? path.dirname(socketRoot || '') === socketNamespace.root && /^s-[0-9a-f]{10}$/.test(path.basename(socketRoot || ''))
    : path.dirname(socketRoot || '') === '/tmp' && new RegExp(`^zobs-${parent?.pid}-[0-9a-f]{10}$`).test(path.basename(socketRoot || ''));
  if (typeof socketRoot !== 'string' || path.resolve(socketRoot) !== socketRoot || !validPath || socketOwnerFile !== expectedOwnerFile) {
    throw new Error('socket root does not match the exact managed /tmp contract');
  }
  if (socketNamespace && !await validateSocketNamespace(socketNamespace, path.dirname(root))) return false;
  try { await requirePrivateDirectory(socketRoot); }
  catch (error) { if (error?.code === 'ENOENT') return false; throw error; }
  // Only an atomically retired generation may have interrupted recursive removal.
  // Normal/preparing generations still require the complete owner record.
  if (retired) return true;
  const owner = await readPrivateJson(expectedOwnerFile);
...
export class PosixObserverContainment extends EventEmitter {
  constructor({
    dataDir,
  ...
  async _privateEnvironment(root, socketRoot) {
    ...
    const darwin = this.helperPlatform.startsWith('darwin-');
    const cache = darwin
      ? path.join(home, 'Library', 'Caches', 'org.Zellij-Contributors.Zellij')
      : path.join(root, 'cache');
    const zellijData = darwin
  ...
  async _socketNamespace(create = false) {
    const pointer = path.join(path.dirname(this.runtimeRoot), 'socket-namespace.json');
    let namespace;
    try { namespace = await readPrivateJson(pointer); }
    catch (error) { if (error?.code !== 'ENOENT') throw error; }
    if (namespace && await validateSocketNamespace(namespace, this.runtimeRoot)) return namespace;
    if (!create) return null;
    if ((await fs.promises.readdir(this.runtimeRoot)).some((name) => !/^\.preparing-[A-Za-z0-9]{6}$/.test(name))) {
      throw new Error('missing socket namespace pointer with persisted generations');
    }
    // mkdtemp never adopts a pre-existing directory. Until the pointer is
    // published this is an inert orphan: no generation or child can use it.
    namespace = { root: await fs.promises.mkdtemp('/tmp/zobs-'), token: crypto.randomBytes(16).toString('hex') };
    await fs.promises.writeFile(path.join(namespace.root, '.observer-namespace.json'),
      JSON.stringify({ runtimeRoot: this.runtimeRoot, token: namespace.token }), { flag: 'wx', mode: 0o600 });
    await writeJsonAtomic(pointer, namespace);
    await validateSocketNamespace(namespace, this.runtimeRoot);
    return namespace;
  }

  async _removeRetired(retiredRoot, state) {
    try { await requirePrivateDirectory(retiredRoot); }
    catch (error) { if (error?.code === 'ENOENT') return; throw error; }
    if (state && await validatePersistedSocketRoot(state, true)) {
      await fs.promises.rm(state.socketRoot, { recursive: true, force: true });
    }
    // State can disappear only in this second removal, after socket cleanup.
    await fs.promises.rm(retiredRoot, { recursive: true, force: true });
  }

  async _retire(state, sourceRoot = state.root) {
    const retiredRoot = path.join(this.runtimeRoot, `.retired-${path.basename(state.root)}`);
    await fs.promises.rename(sourceRoot, retiredRoot);
    state.retiredRoot = retiredRoot;
    await this._removeRetired(retiredRoot, state);
  }
  ...
  async startGeneration({ generation, binaryPath, runtime }) {
    ...
    const socketNamespace = await this._socketNamespace(true);
    const preparingRoot = await fs.promises.mkdtemp(path.join(this.runtimeRoot, '.preparing-'));
    const socketRoot = path.join(socketNamespace.root, `s-${crypto.randomBytes(5).toString('hex')}`);
    const socketPreparing = await fs.promises.mkdtemp(path.join(socketNamespace.root, '.preparing-'));
    ...
    await fs.promises.writeFile(path.join(preparingRoot, 'ownership.marker'), `${nonce}\n`, { flag: 'wx', mode: 0o600 });
    await fs.promises.writeFile(path.join(socketPreparing, '.observer-owner.json'), `${JSON.stringify({ nonce, marker, root, socketRoot })}\n`, {
      flag: 'wx', mode: 0o600,
    });
    ...
    await fs.promises.writeFile(path.join(preparingRoot, 'readonly-tmux.kdl'), layout, { flag: 'wx', mode: 0o600 });
    const parent = await this._identity(process.pid);
    const persisted = {
      schema: 1, generation, nonce, parent, marker, markerFile, root, socketRoot, socketOwnerFile, socketNamespace,
      sessionName, port, target, state: 'starting',
    };
    await writeJsonAtomic(path.join(preparingRoot, 'state.json'), persisted);
    // A collision is never adopted or removed. The authenticated namespace is
    // private to this runtime; serialized lifecycle operations own publication.
    try { await fs.promises.lstat(socketRoot); throw new Error('socket publication collision'); }
    catch (error) { if (error?.code !== 'ENOENT') throw error; }
    await fs.promises.rename(socketPreparing, socketRoot);
    await fs.promises.rename(preparingRoot, root);
    const active = {
      ...persisted, stateFile, tokenFile, guardian: null, guardianLiveness: null,
    ...
    try {
      ...
      active.guardianLiveness = guardian.stdio[3];
  ...
  async _stopGeneration({ reason = 'stop' } = {}) {
    ...
    if (active.retiredRoot) {
      await this._removeRetired(active.retiredRoot, active);
      this.active = null;
      return { stopped: true, reason, count: 0 };
    }
  ...
  async reconcilePersisted() {
    let entries;
    try {
      const runtimeStat = await fs.promises.lstat(this.runtimeRoot);
      if (!runtimeStat.isDirectory() || runtimeStat.isSymbolicLink() || (runtimeStat.mode & 0o022) !== 0 ||
        ...
        throw new ObserverContainmentError('reconcile_failed', 'Unsafe Observer generations directory');
      }
    ...
    const namespace = await this._socketNamespace();
    if (namespace) {
      for (const entry of await fs.promises.readdir(namespace.root, { withFileTypes: true })) {
        if (/^\.preparing-[A-Za-z0-9]{6}$/.test(entry.name)) {
          const staging = path.join(namespace.root, entry.name);
          await requirePrivateDirectory(staging);
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const preparing = /^\.preparing-[A-Za-z0-9]{6}$/.test(entry.name);
        const retired = /^\.retired-[^.][A-Za-z0-9-]*$/.test(entry.name);
        const statePath = path.join(root, 'state.json');
        if (preparing || retired) {
          try { state = await readPrivateJson(statePath); }
          catch (error) { if (error?.code !== 'ENOENT') throw error; }
          if (state) {
            const expectedRoot = retired ? path.join(this.runtimeRoot, entry.name.slice('.retired-'.length)) : state.root;
            if (state.root !== expectedRoot || path.dirname(state.root) !== this.runtimeRoot ||
                state.markerFile !== path.join(state.root, 'ownership.marker') || state.marker !== `fdpath:${state.markerFile}`) {
              throw new Error('staged state paths do not match managed generation');
            }
            if (retired) await this._removeRetired(root, state);
            else {
              await validatePersistedSocketRoot(state);
              await this._retire(state, root);
            }
          } else {
