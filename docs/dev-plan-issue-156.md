...
- [ ] **Registry validation as a real step (not just an Assumption):** validate each entry (name non-empty/unique, base_url well-formed, key present). At load/poll time, probe capability and record a per-agent `health_reason` (`ok` / `version_unsupported` (no `/api/auth/token`) / `unreachable` / `auth_failed`). The ">= #146" and "network reachable" preconditions surface here as agent state, not silent assumptions.
- [ ] Unit tests: identity field present/typed in `/api/state`; registry parse/validation (missing fields, dup names, empty list, malformed URL).
...
- [ ] Staleness/liveness: failed poll → `unreachable`; missing token endpoint → `version_unsupported`; stale `last_seen` → `offline`; distinguish all from `idle`. TTL on cached records.
...
- [ ] Phase 0: `/api/state` identity field; registry validation edge cases; `health_reason` assignment
...
- [ ] Phase 0: `/api/state` exposes stable agent identity; registry loads, validates, and assigns `health_reason`
