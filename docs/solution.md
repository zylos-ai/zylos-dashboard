...
| **context_usage** | Context window 使用率 | % | ✅ | ❌ | telemetry → statusLine → status file |
...
|---------|---------|-------|---------|
| **FileAdapter** | activity-monitor JSON/JSONL | Phase 1 | agent_state, current_tool, tool_calls, tool_duration, health, context_usage (Claude only via status file) |
...
4. **Codex context_usage**：无 statusLine 替代。目前标记 unsupported，未来可能通过 Codex API 或状态文件获取
